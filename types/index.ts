...
export interface BoardConfig {
  mode: GameMode;
  pricePerCell: number; // in cents or token units
  entryFee: number; // in cents or token units
  prizePool: number; // in cents or token units
  startTime: string | Date; // ISO timestamp or Date
  homeTeam: BoardTeam;
  awayTeam: BoardTeam;
  sport: typeof SPORTS_LEAGUES[keyof typeof SPORTS_LEAGUES];
  externalGameId?: string;
  gameStartTime?: string; // ISO timestamp
  tokenSymbol?: 'ALSA' | 'SPIRIT'; // Token used for payments
}
...
export interface UserStats {
  ...
  totalCells: number;
  totalWins: number;
  totalEarnings: number; // in cents or token units
}
...
export interface Payment {
  ...
  userId: string;
  boardId: string;
  amount: number; // in cents or token units
  currency: string;
