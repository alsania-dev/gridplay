...
import { THEME_COLORS, GAME_MODES, BOARD_STATUS } from '../lib/constants';
...
export default function ShotgunPage() {
  ...
  const handleCellClaim = async (index: number) => {
    ...
    if (game.id !== 'demo-shotgun') {
      try {
        // Convert linear index to row/col for shotgun (2 rows of 10)
        const row = Math.floor(index / 10);
        const col = index % 10;
        await claimCell(game.id, row, col, userId, displayName);
      } catch (err) {
  ...
  const handleLockIn = async () => {
    ...
    // Generate scores
    const scoreResult = await generateScores(game.id);
    ...
    // Calculate winners
    const winnerResult = await calculateWinners(game.id, 0, 0);
    if (winnerResult.success && winnerResult.data) {
      // For shotgun, we need to determine halftime and final winners
      // This is a simplified version
      setWinners({
        halftime: winnerResult.data[0] ? 0 : null,
        final: winnerResult.data[1] ? 10 : null,
      });
    }
