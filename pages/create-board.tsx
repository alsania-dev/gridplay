 ...
 * 
 * Form to create new game boards with token-based entry fees.
 * Supports ALSA and SPIRIT tokens.
 */
...
import { GAME_MODES, THEME_COLORS, ENTRY_FEES, PAYOUT_STRUCTURE, SPORTS_LEAGUES } from '../lib/constants';
...
// Escrow/Game contract address (same as hi-lo for now - update later)
...
export default function CreateBoard() {
  ...
  const selectedTokenMeta = TOKEN_METADATA[formData.tokenSymbol];
  ...
  const handlePaymentSuccess = async (transactionHash: string) => {
    ...
    try {
      ...
      let userId = localStorage.getItem('gridplay_user_id');
      if (!userId) {
        userId = `user_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        localStorage.setItem('gridplay_user_id', userId);
      }

      // Create the board in database
      const result = await createGame({
        name: formData.name,
        mode: formData.gameMode,
        pricePerCell: formData.entryFee,
        homeTeamId: formData.homeTeam,
        awayTeamId: formData.awayTeam,
        sport: SPORTS_LEAGUES.FOOTBALL,
        gameStartTime: formData.startTime || undefined,
      }, userId);

      if (result.success && result.data) {
        setCreatedBoardId(result.data.id);
      } else {
        setError(result.error?.message || 'Board created but database save failed. Please contact support.');
      }
    } catch (err) {
      console.error('Error creating board:', err);
      setError('Board created but database save failed. Please contact support with your transaction hash.');
    } finally {
  ...
  // Handle form submission (triggers payment flow)
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Payment is handled by TokenPayment component
    // The form will wait for payment to complete
  };
  ...
  if (createdBoardId) {
    return (
      <Layout title="Board Created - GridPlay" showWallet={true}>
        <div className="min-h-[80vh] flex items-center justify-center px-4">
          ...
          >
            ...
            {paymentTxHash && (
              <div 
                className="p-4 rounded mb-6 text-left"
                style={{ backgroundColor: '#0A0A0A' }}
              >
                <p className="text-xs text-[#9CA3AF] mb-2">Transaction Hash:</p>
                <p className="font-mono text-xs break-all text-[#10B981]">
                  {paymentTxHash}
                </p>
              </div>
            )}
  ...
  return (
    <Layout title="Create Board - GridPlay" showWallet={true}>
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[#9CA3AF]">
            Set up your game board with token entry fees
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(['ALSA', 'SPIRIT'] as TokenSymbol[]).map((symbol) => (
                  <button
                    key={symbol}
                    type="button"
                    onClick={() => setFormData(prev => ({ ...prev, tokenSymbol: symbol }))}
                    className={`
                      p-4 rounded-lg border text-center transition-all
                      ${formData.tokenSymbol === symbol ? 'ring-2 ring-[#10B981]' : ''}
                    `}
                    style={{ 
                      backgroundColor: formData.tokenSymbol === symbol ? 'rgba(16, 185, 129, 0.1)' : '#0A0A0A',
                      borderColor: formData.tokenSymbol === symbol ? THEME_COLORS.PRIMARY : '#374151',
                    }}
                  >
                    <div className="text-2xl mb-1">{TOKEN_METADATA[symbol].icon}</div>
                    <div 
                      className="font-bold"
                      style={{ color: THEME_COLORS.TEXT }}
                    >
                    ...
                    <div className="text-xs text-[#9CA3AF]">{TOKEN_METADATA[symbol].name}</div>
                    {isConnected && (
                      <div className="text-xs mt-1 text-[#10B981]">
                        Balance: {formattedBalances[symbol]}
                      </div>
                    )}
                  </button>
                ))}
            ...
            <div className="mb-6">
              <label 
                ...
                style={{ color: THEME_COLORS.TEXT }}
              >
                Entry Fee per Cell ({selectedTokenMeta.symbol})
              </label>
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">{selectedTokenMeta.icon} {formData.entryFee} {selectedTokenMeta.symbol}</span>
              </div>
              <div className="flex justify-between border-t border-[#374151] pt-3">
                ...
                >
                  {selectedTokenMeta.icon} {totalEntryFee} {selectedTokenMeta.symbol}
                </span>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  ...
                  >
                    ...
                    <div 
                      ...
                      style={{ color: THEME_COLORS.ACCENT }}
                    >
                      {selectedTokenMeta.icon} {payouts.first}
                    </div>
                  </div>
                  <div 
                    className="p-2 rounded"
                  ...
                  >
                    ...
                    <div className="font-bold text-[#C0C0C0]">{selectedTokenMeta.icon} {payouts.second}</div>
                  </div>
                  <div 
                    className="p-2 rounded"
                  ...
                  >
                    ...
                    <div className="font-bold text-[#CD7F32]">{selectedTokenMeta.icon} {payouts.third}</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* Payment Section */}
          {isConnected && isCorrectNetwork && !paymentComplete && (
            <div className="mb-6">
              <TokenPayment
                ...
                buttonText="Create Board & Pay"
                disabled={isSubmitting}
              />
            </div>
          )}

          {/* Submit Button (when not using TokenPayment) */}
          {(!isConnected || !isCorrectNetwork || paymentComplete) && (
            <div className="flex gap-4">
              ...
              >
                {isSubmitting ? 'Processing...' : 'Create Board'}
              </Button>
            </div>
          )}
        </form>
